...
% Call the function
[T20, U20, W20, uw20, ww20, ut20, u20, w20, CumSec20] = get_data(data_dir, towers, '20m');
[T10, U10, W10, uw10, ww10, ut10, u10, w10, CumSec10] = get_data(data_dir, towers(1:4), '10m');
[T3, U3, W3, uw3, ww3, ut3, u3, w3, CumSec3] = get_data(data_dir, towers, '3m');

% Function Definition (for example, if not already implemented)
function [T, U, W, uw, ww, ut, u, w, CumSec] = get_data(dr, towers, height)
    ...
    for ti=1:length(towers)
        ...
        for di = 1:3
            ...
            ut{di, ti} = df{di}.("u't' (mC/s)");
            u{di, ti} = df{di}.("U (m/s)") - mean(df{di}.("U (m/s)"), 'omitmissing');
            w{di, ti} = df{di}.("w' (m/s)");
...
for di=1:3
    for ti=1:4
        buoy{di, ti, 1} = calc_1min_buoy(T3{di,ti}, ut3{di,ti}, samplingfreq, CumSec3{di, ti}, CumSec_n{di, ti});
        shear{di, ti, 1} = calc_1min_shear("forward", uw3{di,ti}, ww3{di,ti}, U3{di,ti}, U10{di,ti}, U20{di,ti}, W3{di,ti}, W10{di,ti}, W20{di,ti}, height, samplingfreq, CumSec3{di, ti}, CumSec_n{di, ti});
        transport{di, ti, 1} = calc_1min_transport("forward", w3{di,ti}, w10{di,ti}, w20{di,ti}, u3{di,ti}, u10{di,ti}, u20{di,ti}, height, samplingfreq, CumSec3{di, ti}, CumSec10{di, ti}, CumSec20{di, ti}, CumSec_n{di, ti});
        
        buoy{di, ti, 2} = calc_1min_buoy(T10{di,ti}, ut10{di,ti}, samplingfreq, CumSec10{di, ti}, CumSec_n{di, ti});
        shear{di, ti, 2} = calc_1min_shear("center", uw10{di,ti}, ww10{di,ti}, U3{di,ti}, U10{di,ti}, U20{di,ti}, W3{di,ti}, W10{di,ti}, W20{di,ti}, height, samplingfreq, CumSec10{di, ti}, CumSec_n{di, ti});
        transport{di, ti, 2} = calc_1min_transport("center", w3{di,ti}, w10{di,ti}, w20{di,ti}, u3{di,ti}, u10{di,ti}, u20{di,ti}, height, samplingfreq, CumSec3{di, ti}, CumSec10{di, ti}, CumSec20{di, ti}, CumSec_n{di, ti});
        
        buoy{di, ti, 3} = calc_1min_buoy(T20{di,ti}, ut20{di,ti}, samplingfreq, CumSec20{di, ti}, CumSec_n{di, ti});
        shear{di, ti, 3} = calc_1min_shear("backward", uw20{di,ti}, ww20{di,ti}, U3{di,ti}, U10{di,ti}, U20{di,ti}, W3{di,ti}, W10{di,ti}, W20{di,ti}, height, samplingfreq, CumSec20{di, ti}, CumSec_n{di, ti});
        transport{di, ti, 3} = calc_1min_transport("backward", w3{di,ti}, w10{di,ti}, w20{di,ti}, u3{di,ti}, u10{di,ti}, u20{di,ti}, height, samplingfreq, CumSec3{di, ti}, CumSec10{di, ti}, CumSec20{di, ti}, CumSec_n{di, ti});
...
function shear = calc_1min_shear(grad_method, uwi, wwi, U1, U2, U3, W1, W2, W3, height, samplingfreq, timei, timen)
    ...
    shear_temp = -uwi .* grad_W -wwi .* grad_U;
    shear = calc_1min_ave(shear_temp, samplingfreq);
end


function transport = calc_1min_transport(grad_method, w1, w2, w3, u1, u2, u3, height, samplingfreq, time1, time2, time3, timen)
    if length(time1)~=length(timen)
        if length(time1)>length(timen)
            w1 = w1(1:length(timen));
            u1 = u1(1:length(timen));
        else
            w1 = interp1(time1, w1, timen, 'linear', 'extrap');
            u1 = interp1(time1, u1, timen, 'linear', 'extrap');
        end
    end
    ...
    if length(time3)~=length(timen)
        if length(time3)>length(timen)
            ...
            u3 = u3(1:length(timen));
        else
            w3 = interp1(time3, w3, timen, 'linear', 'extrap');
            u3 = interp1(time3, u3, timen, 'linear', 'extrap');
        end
    end
    ...
    if strcmp(grad_method, "center") % 10m
        ...
        weight1 = (height(3)-height(2)) / (height(3)-height(1));
        weight3 = (height(2)-height(1)) / (height(3)-height(1));
        transport_temp = weight1 .* (w2.*w2.*u2 - w1.*w1.*u1)./(height(2)-height(1)) + weight3 .* (w3.*w3.*u3 - w2.*w2.*u2)./(height(3)-height(2));
    elseif strcmp(grad_method, "forward")
        transport_temp = (w2.*w2.*u2 - w1.*w1.*u1)./(height(2)-height(1));
    elseif strcmp(grad_method, "backward")
        transport_temp = (w3.*w3.*u3 - w2.*w2.*u2)./(height(3)-height(2));
    end
    transport = calc_1min_ave(transport_temp, samplingfreq);
end

function buoy = calc_1min_buoy(Tmi, uti, samplingfreq, timei, timen)
    if length(timei)~=length(timen)
        if length(timei)>length(timen)
            uti = uti(1:length(timen));
        else
            uti = interp1(timei, uti, timen, 'linear', 'extrap');
        end
    end
    g = 9.81;  %m/s^2 gravity
    buoy_temp = g ./ (Tmi + 273.15) .* uti;
...
for ti=1:4
    for hi=1:3
        ...
        if pi==1
            legend('buoyancy', 'shear', 'transport', 'box', 'off', 'Orientation','horizontal', "FontSize",20)
        end
        if pi==5
            ylabel("{\boldmath$\mathrm{\overline{u'w'}\hspace{1.7mm}Budget\hspace{1.7mm}Components\hspace{1.7mm}[m^2\hspace{1.7mm}s^{-3}]}$}", "FontSize",20, 'Interpreter','latex')
        end
    end
end