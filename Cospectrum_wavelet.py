...
def get_plot(vn, fn, psd_pre, psd_ffp, psd_pos):
    ...
    plt.subplots_adjust(top=.95, bottom=.05, right=.95, left=.1,
                        hspace=.1, wspace=0)
    plt.savefig(f'{box_path}/Diane/WORK/SERDP/plot/spectral/EastTower_CoSpectrum{fn}_Averaged_loglog_smoonthed.png', bbox_inches='tight')
    plt.close()
...
def get_normalized_cospectraplot(vn, fn, phi, psdtot, method):
    ...
    plt.subplots_adjust(top=.95, bottom=.05, right=.95, left=.1,
                        hspace=.1, wspace=.08)
    if method:
        plt.savefig(f'{box_path}/Diane/WORK/SERDP/plot/spectral/EastTower_NormCoSpectrum{fn}_logx_smoonthed.png',
                    bbox_inches='tight')
    else:
        plt.savefig(f'{box_path}/Diane/WORK/SERDP/plot/PeriodMean_EastTower_NormCoSpectrum{fn}_logx_smoonthed.png',
                    bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    df = pd.read_table(box_path+'/Diane/WORK/SERDP/data/FreqForSmooth.txt', delim_whitespace=True,
    ...
    dfpre = pd.read_csv(box_path+'/Diane/WORK/SERDP/data/East_Pre-FFP.csv')
    dfffp = pd.read_csv(box_path+'/Diane/WORK/SERDP/data/East_FFP.csv')
    dfpos = pd.read_csv(box_path+'/Diane/WORK/SERDP/data/East_Post-FFP.csv')
    method = True
